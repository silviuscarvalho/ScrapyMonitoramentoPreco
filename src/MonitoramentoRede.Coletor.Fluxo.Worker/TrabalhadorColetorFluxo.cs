...
namespace MonitoramentoRede.Coletor.Fluxo.Worker;

public sealed class TrabalhadorColetorFluxo : BackgroundService
{
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly ILogger<TrabalhadorColetorFluxo> _logger;
    private readonly OpcaoApiInterna _apiInterna;
    private readonly OpcaoColetorArquivos _coletor;

    public TrabalhadorColetorFluxo(
        IHttpClientFactory httpClientFactory,
        IOptions<OpcaoApiInterna> apiInterna,
        IOptions<OpcaoColetorArquivos> coletor,
        ILogger<TrabalhadorColetorFluxo> logger)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            foreach (var arquivo in arquivos)
            {
                try
                {
                    ...
                    var payload = JsonSerializer.Deserialize<FluxoRedeEntradaDto>(json, new JsonSerializerOptions(JsonSerializerDefaults.Web))
                        ?? throw new InvalidOperationException("Arquivo de fluxo inválido.");

                    var cliente = _httpClientFactory.CreateClient();
                    cliente.DefaultRequestHeaders.Add("X-Api-Key", _apiInterna.Chave);
                    var resposta = await cliente.PostAsJsonAsync($"{_apiInterna.UrlBase}/api/interno/ingestao/fluxos", payload, stoppingToken);
                ...
                {
                    _logger.LogError(ex, "Erro ao processar arquivo de fluxo {Arquivo}", arquivo);
