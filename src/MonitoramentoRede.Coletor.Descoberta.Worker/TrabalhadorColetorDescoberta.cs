...
namespace MonitoramentoRede.Coletor.Descoberta.Worker;

public sealed class TrabalhadorColetorDescoberta : BackgroundService
{
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly ILogger<TrabalhadorColetorDescoberta> _logger;
    private readonly OpcaoApiInterna _apiInterna;
    private readonly OpcaoColetorArquivos _coletor;

    public TrabalhadorColetorDescoberta(
        IHttpClientFactory httpClientFactory,
        IOptions<OpcaoApiInterna> apiInterna,
        IOptions<OpcaoColetorArquivos> coletor,
        ILogger<TrabalhadorColetorDescoberta> logger)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            foreach (var arquivo in arquivos)
            {
                try
                {
                    ...
                    var payload = JsonSerializer.Deserialize<DispositivoDetectadoEntradaDto>(json, new JsonSerializerOptions(JsonSerializerDefaults.Web))
                        ?? throw new InvalidOperationException("Arquivo de descoberta inválido.");

                    var cliente = _httpClientFactory.CreateClient();
                    cliente.DefaultRequestHeaders.Add("X-Api-Key", _apiInterna.Chave);
                    var resposta = await cliente.PostAsJsonAsync($"{_apiInterna.UrlBase}/api/interno/ingestao/dispositivos", payload, stoppingToken);
                ...
                {
                    _logger.LogError(ex, "Erro ao processar arquivo de descoberta {Arquivo}", arquivo);
